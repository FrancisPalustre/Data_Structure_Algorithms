...
//Iterative DFS
vector<string> DFS(map<string, vector<string> > graph, string startingNode, string desiredNode) {
    //creating containers of visited and stack values
    stack<vector<string> > stack;
    vector<string> visited;

   //starting the stack with our starting node
    stack.push(vector<string>(1, startingNode));

    while (!stack.empty()) {
        vector<string> path = stack.top(); //set top item in stack
        ...
        if (findCurr == visited.end()) {
            visited.push_back(currNode); //mark as visited

            //if desired node is found, return path
            if (currNode == desiredNode) {
                return path;
            }

            vector<string> adjNodes = graph[currNode];
            vector<string>::iterator neighbor = adjNodes.begin();
            for (neighbor; neighbor != adjNodes.end(); neighbor++) { //iterates the adjNodes
                
                vector<string>::iterator findVisit = find(visited.begin(), visited.end(), *neighbor);
                if (findVisit  == visited.end()) {
                    vector<string> newPath = path;
                    
                    newPath.push_back(*neighbor); //add neighbor to newPath vector
                    stack.push(newPath); //adds new path to stack
                }
            }
        }
    }
